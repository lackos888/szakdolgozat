...
function module.trySSLCertificateCreation(method, domain, webserverType)
    ...
    if method == "dns" then
        ...
        local formattedCmd = "(certbot certonly --noninteractive "..tostring(dryRunStr).." --agree-tos --register-unsafely-without-email --no-eff-email --manual --preferred-challenges dns --manual-auth-hook \"sh ./authenticator.sh \""..tostring(tempFileName).."\"\" -d "..tostring(domain).." > \""..tostring(tempFileNameForStdOut).."\" 2>&1; echo $? > \""..tostring(tempFileName).."\") & echo $! > certbot_pid.txt";
